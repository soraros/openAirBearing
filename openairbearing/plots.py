    ...
    if result.p.ndim == 2:
      ...
      for h_plot, t_loc in zip(h_plots, t_locations):
        ...
        fig.add_trace(
          go.Scatter(
            ...
            text=[
              f"{h_plot * 1e6:.2f} μm" if i == t_loc else None for i in range(b.nx)
            ],
      ...
      for i, h in enumerate(bearing.ha * 1e6):
        step = dict(
          ...
          args=[
            {"z": [pressures[:, :, i]]},
            {"title": f"Pressure Distribution (h = {h:.1f})"},  # Update the title
          ],
          label=f"{h:.1f} μm",
        )
        steps.append(step)
...
def plot_supply_flow_rate(bearing, results):
  ...
  fig.update_xaxes(title_text="h (μm)", range=[0, b.ha_max * 1e6], **AXIS_STYLE)
  fig.update_yaxes(title_text="q<sub>s</sub> (L/min)", range=[None, None], **AXIS_STYLE)
...
def plot_chamber_flow_rate(bearing, results):
  ...
  fig.update_xaxes(title_text="h (μm)", range=[0, b.ha_max * 1e6], **AXIS_STYLE)
  fig.update_yaxes(title_text="q<sub>c</sub> (L/min)", range=[None, None], **AXIS_STYLE)
